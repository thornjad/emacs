...
*** Buffer terminator
Automatically kills old unused buffers to prevent memory bloat and keep buffer list clean.

#+BEGIN_SRC emacs-lisp :lexical t
  (package! buffer-terminator :auto
    :custom
    (buffer-terminator-verbose nil)
    (buffer-terminator-inactivity-timeout (* 36 60 60)) ; 36 hours in seconds
    (buffer-terminator-check-interval (* 3 60 60))      ; Check every 3 hours
    :config
...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (defun aero/thornlog-tickets ()
    "Run an AI Agent with /tickets command in thornlog directory."
    (interactive)
    (let ((default-directory aero/thornlog-path))
      (async-shell-command "claude --print /tickets" "*Claude Tickets*")))
    ...
    (let ((today-heading (format "* %s" (format-time-string "%A, %B %d"))))
      (save-excursion
        ...
        (if (search-forward today-heading nil t)
            ;; Entry for today already exists, just go to end of file
            (goto-char (point-max))
...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (defun thornlog-mcp-create-ticket (title &optional priority scheduled tags body)
    "Create a new TICKET task in the main todo file via MCP.
  TITLE is required. PRIORITY defaults to E. SCHEDULED defaults to today.
  TAGS should be a string like 'tag1:tag2' or nil. BODY is optional content to add after the properties."
    (let ((todo-file (expand-file-name "todo.org" aero/roam-path))
          (priority-str (or priority "E"))
  ...
  Valid states: TODO, WAITING, BLOCKED, BACKLOG, DONE, REMOVED, TICKET, PR, CLOSED, REVIEW"
    (let ((todo-file (expand-file-name "todo.org" aero/roam-path))
          (valid-states '("TODO" "WAITING" "BLOCKED" "BACKLOG" "DONE" "REMOVED"
                          "TICKET" "PR" "CLOSED" "REVIEW")))

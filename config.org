...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (defun aero/thornlog-clean-save ()
    ...
    (let* ((default-directory aero/thornlog-path)
      ...
      (when (boundp 'aero/gdrive-thornlog-base)
      ...
      (let ((existing-task (gethash key existing-by-ticket)))
        (if existing-task
            ...
            (let ((current-state (alist-get 'state existing-task)))
              (let ((current-state-clean (substring-no-properties current-state)))
                ;; Only update state if the new state is more restrictive or if current state is TODO
                ;; This preserves manually set BLOCKED states unless Jira indicates the ticket is done
                (when (and (not (equal current-state-clean org-state))
                           (or (equal current-state-clean "TODO")
                               (and (member current-state-clean '("BLOCKED" "WAITING"))
                                    (member org-state '("DONE" "PR")))))

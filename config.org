...
And a helper that lets us check the MELPA recipe for a package, which is useful when setting up new packages sometimes, and slightly faster than searching on a browser.
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-minor-mode aero-breadcrumb-local-mode
    ...
    :init-value nil
    (if aero-breadcrumb-local-mode

        (setq header-line-format (delete
                                  '(:eval (aero-breadcrumb--header-line))
                                  header-line-format))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org-roam (:repo "org-roam/org-roam" :files (:defaults "extensions/*"))
    :defer 1
    :after (general org)
...
Enabled in all programming modes, and org-mode, except we don't want it during org-capture.
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! copilot (:host github :repo "zerolfx/copilot.el" :files ("dist" "*.el"))
    ...
    :config
    (add-hook 'org-capture-mode-hook (lambda () (copilot-mode -1))))
#+END_SRC

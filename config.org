...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (package! spinner :auto) ; required by cider
#+END_SRC
...
Buffer previews are disabled since they slow down navigation.
...
Truncates long names to ensure other modeline segments remain visible."
    (let* ((name (if (and (derived-mode-p 'org-mode)
                          (buffer-file-name)
                          (org-roam-file-p))
                     (or (ignore-errors
                           (caar (org-roam-db-query
...
#+begin_src emacs-lisp :lexical t
  (package! tao-theme "11111000000/tao-theme-emacs")
...
Required by transient (used by magit). Not yet in MELPA, so we pull from GitHub.

#+BEGIN_SRC emacs-lisp :lexical t
  ;; cond-let is required by magit (magit-section.el, magit-base.el)
  (package! cond-let "tarsius/cond-let"
    :defer t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! magit :auto
    ...
    ;; Force Magit to ALWAYS use the current window - no new windows or frames ever
    (add-to-list 'display-buffer-alist
                 '("\\`\\*magit.*\\*\\'"
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org :builtin
    ...
    :custom
    ;; don't persist the element cache to disk; stale cache + kqueue events on
    ;; wake from sleep causes incorrect buffer modifications in org files
...
With 400+ nodes synced across multiple computers via Syncthing, database performance is critical. The default SQLite configuration is not optimized for this workload, leading to 2-4 minute sync times that completely break flow.

The key optimization is enabling Write-Ahead Logging (WAL) mode. WAL allows concurrent reads during writes, reduces locking contention, and significantly improves write performance. Combined with a larger cache (increased from ~8MB to ~40MB), this provides 10-50x speedup for database operations. The synchronous mode is set to NORMAL (instead of FULL) for better performance while maintaining safety, and temporary tables are stored in memory rather than on disk.

#+BEGIN_SRC emacs-lisp :lexical t
  (defun aero/org-roam-db-optimize ()
    "optimize org-roam database for better performance with WAL mode and larger cache."
    (when (and org-roam-db-connection
               (emacsql-live-p org-roam-db-connection))
      (emacsql org-roam-db-connection [:pragma (= journal_mode WAL)])
      (emacsql org-roam-db-connection [:pragma (= cache_size 10000)])
      (emacsql org-roam-db-connection [:pragma (= synchronous NORMAL)])
      (emacsql org-roam-db-connection [:pragma (= temp_store MEMORY)])))

  (advice-add 'org-roam-db--init :after #'aero/org-roam-db-optimize)
#+END_SRC

*** Automatic incremental syncing

When Syncthing adds a new node file from another computer, the org-roam database doesn't know about it until a sync occurs. The default autosync-mode uses file system watchers, but these don't always catch changes from Syncthing reliably. Additionally, manual sync with ~org-roam-db-sync~ defaults to a full scan of all files, taking 2-4 minutes.

This implementation adds an idle timer that runs incremental syncs every 30 seconds of idle time. Incremental sync (without the ~'force~ flag) only processes files that have changed since the last sync, typically completing in under 1 second. This catches Syncthing changes automatically without manual intervention, and the idle timer ensures it never interrupts active work.
...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (setq aero/org-roam-idle-sync-timer
        (run-with-idle-timer 30 t #'aero/org-roam-incremental-sync))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  (defun aero/org-roam-db-vacuum ()
    ...
    (when (and org-roam-db-connection
      ...
      (emacsql org-roam-db-connection [:vacuum])
      (emacsql org-roam-db-connection [:analyze])
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org-roam-latte "yad-tahir/org-roam-latte"
    :after org-roam
...
#+BEGIN_SRC emacs-lisp :lexical t
    (package! flyspell :builtin
      ...
      (setq
       ...
       ispell-extra-args '("-d" "en_US,fr_FR")
      ...
      ;; Support #+localwords: in org file headers as an alternative to
      ;; LocalWords: at end of file. Case-insensitive match; writes lowercase.

      (defun aero/org-header-end ()
        "Return position just after the #+keyword block in the preamble.
  Uses org-roam-end-of-meta-data which handles all edge cases."
      ...
      (defun aero/org-collect-localwords (&optional limit)
        "Return words from #+localwords: lines in the org file header."
        (let (words)
          (save-excursion
            (save-match-data
              (goto-char (point-min))
              (let ((limit (or limit (aero/org-header-end)))
                    (case-fold-search t))
                (while (re-search-forward "^#[+]localwords:\\s-*\\(.+\\)$" limit t)
                  (setq words (append words (split-string (match-string-no-properties 1))))))
              words))))

      (defun aero/org-ispell-inject-localwords (&rest _)
        "Send #+localwords: header entries to the running ispell process.
  Handles the persistent-process path used by `ispell-word' and the
  small-region flyspell path."
        (when (derived-mode-p 'org-mode)
          (dolist (word (aero/org-collect-localwords))
            (ispell-send-string (concat "@" word "\n")))))

      (advice-add 'ispell-buffer-local-words :after #'aero/org-ispell-inject-localwords)

      (defun aero/org-ispell-add-to-header (word)
        "Add WORD to #+localwords: in the org header, preserving case."
        (let* ((header-end (aero/org-header-end)))
          (unless (member-ignore-case word (aero/org-collect-localwords header-end))
            (save-excursion
              (save-match-data
                (let ((case-fold-search t)
                      last-eol)
                  (goto-char (point-min))
                  (while (re-search-forward "^#[+]localwords:[^\n]*" header-end t)
                    (setq last-eol (point)))
                  (if last-eol
                      (progn (goto-char last-eol) (insert " " word))
      ...
      (defun aero/org-ispell-add-word-advice (orig-fn word)
        "In org buffers, redirect local word saves to the file header."
        (let* ((target (and ispell-buffer-local-name
                            (get-buffer ispell-buffer-local-name)))
               (org-p (if target
          ...
          (if org-p
              (with-current-buffer (or target (current-buffer))
                (aero/org-ispell-add-to-header word))
            (funcall orig-fn word))))

      (advice-add 'ispell-add-per-file-word-list :around #'aero/org-ispell-add-word-advice)

      ;; the batch path in flyspell-large-region spawns a fresh hunspell
      ;; via call-process-region, so it never sees the @word injections
      ;; sent to the persistent pipe above. we merge localwords into a
      ;; temporary personal dictionary so the batch process picks them up.
      (defun aero/flyspell-batch-with-localwords (orig-fn beg end)
        "Merge #+localwords: into a temp personal dictionary for batch checks."
        (let ((words (and (derived-mode-p 'org-mode)
                          (aero/org-collect-localwords))))
          (if (null words)
              (funcall orig-fn beg end)
            (let* ((src ispell-personal-dictionary)
                   (tmp (make-temp-file "ispell-localwords")))
              (with-temp-file tmp
                (when (and src (file-exists-p src))
                  (insert-file-contents src)
                  (goto-char (point-max))
                  (unless (bolp) (insert "\n")))
                (dolist (w words) (insert w "\n")))
              (let ((ispell-personal-dictionary tmp))
                (unwind-protect
                    (funcall orig-fn beg end)
                  (delete-file tmp)))))))

      (advice-add 'flyspell-large-region :around #'aero/flyspell-batch-with-localwords)
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! ws-butler (:repo "lewang/ws-butler" :branch "master")
    :defer 1
...
** Claude Code IDE
This makes Claude Code work better in Emacs, seems to behave better than my custom vterm handling attempts.

Start by running =claude-code-ide= or with =SPC a c= in the directory you want it. I'll likely change bindings over time as I use this package more.

#+begin_src emacs-lisp :lexical t
  (package! claude-code-ide "manzaltu/claude-code-ide.el"
    :commands (claude-code-ide claude-code-ide-menu)
    :custom
    (claude-code-ide-terminal-backend 'vterm) ; eat has the same problems with less usability, so use vterm
    (claude-code-ide-use-side-window nil) ; use normal window
    (claude-code-ide-diagnostics-backend 'flymake)
    :init (aero-leader-def "ac" 'claude-code-ide-menu)
    :config
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! copilot (:host github :repo "zerolfx/copilot.el" :files ("dist" "*.el"))
    ...
    :commands (copilot-mode copilot-next-completion copilot-previous-completion copilot-accept-completion)
...
* Thanks
A little packages that uses =gh.el= to star all installed packages as a small thanks to the developers of those packages. I don't want this to run automatically, so instead run =M-x thanks= from time to time.

Based on thanks.el by Jakub Kadlčík (https://github.com/FrostyX/thanks), but simplified and made to work with the CLI interface better.

#+begin_src emacs-lisp :lexical t
  (defun aero/thanks--installed-packages ()
    "get names and urls of all installed straight.el packages."
    (let ((packages '()))
        ...
        (maphash
         (lambda (package recipe)
           (let* ((plist recipe)
                  ...
                  (url nil))
             ;; construct github url from recipe
             (when (and repo (or (eq host 'github) (eq host 'GitHub)))
               (setq url (concat "https://github.com/" repo)))
             ;; some packages might have explicit :url
             (when (and (not url) (plist-get plist :url))
               (setq url (plist-get plist :url)))
             (when url
               (push (cons package url) packages))))
         straight--recipe-cache))
      packages))

  (defun aero/thanks--github-project-url? (url)
    "is this url a github project?"
    (and (stringp url)
         (string-prefix-p "https://github.com/" url)
         (> (length (split-string (string-remove-prefix "https://github.com/" url) "/")) 1)))

  (defun aero/thanks ()
    "Say thanks to authors of installed packages by starring their repos."
    (interactive)
    (let* ((packages (aero/thanks--installed-packages))
           (urls (seq-filter #'aero/thanks--github-project-url?
                             (mapcar #'cdr packages)))
           (urls (delete-dups urls)))
      (if (null urls)
          (message "no github packages found to thank")
        (when (y-or-n-p
               (format "do you want to say thanks to %d projects? " (length urls)))
          (seq-do-indexed
           (lambda (url i)
             (let* ((i (1+ i))
                    (path (string-remove-prefix "https://github.com/" url))
                    (path (string-remove-suffix "/" path))
                    (parts (split-string path "/"))
                    (owner (car parts))
                    (project (cadr parts)))
               (when (= (mod i 50) 0)
                 (message "[%d/%d] pausing briefly to respect rate limits..."
                          i (length urls))
                 (sit-for 30))
               (message "[%d/%d] saying thanks to %s/%s" i (length urls) owner project)
               (condition-case err
                   (unless (= 0 (call-process "gh" nil nil nil "api" "-X" "PUT"
                                              (format "/user/starred/%s/%s" owner project)
                                              "--silent"))
                     (message "failed to star %s/%s" owner project))
                 (error (message "error starring %s/%s: %s" owner project err)))))
           urls)
          (message "said thanks to %d projects! you just made somebody's day better."
                   (length urls))))))
#+end_src

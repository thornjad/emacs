...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org :builtin
    ...
    ;; Restore overline on TODO keywords and priorities within headings. org-mode
    ;; applies org-todo with override=t, wiping the heading face's overline from
    ;; those characters. This re-applies it via append (lower priority), so
    ;; org-todo's foreground/height still win but the overline shows through.
    ;; Using font-lock-add-keywords scopes this to org-mode buffers only, keeping
    ;; org-agenda unaffected.
    (defun aero/org-heading-overline-at-point ()
      (save-excursion
        (beginning-of-line)
        (when (looking-at "\\(\\*+\\)")
          (let* ((level (min (length (match-string 1)) 8))
                 (color (face-attribute (intern (format "org-level-%d" level))
              ...
              (lambda ()
                (font-lock-add-keywords
                 ...
                 `((,(concat "^\\*+ +\\(" org-todo-regexp "\\)")
                    (1 (aero/org-heading-overline-at-point) append t))
                   ("^\\*+ +\\(?:[A-Z][A-Z0-9_-]* +\\)?\\(\\[#[A-Z0-9]\\]\\)"
                    (1 (aero/org-heading-overline-at-point) append t)))
                 'set)))

...
#+BEGIN_SRC emacs-lisp :lexical t
  ...
  properties drawers, missing keywords, files without headings."
        (save-excursion
          (goto-char (point-min))
      ...
      (defun aero/org-ispell-add-to-header (word)
        "Add WORD (lowercased) to #+localwords: in the org header."
        (let* ((word (downcase word))
               (header-end (aero/org-header-end)))
          (unless (member word (aero/org-collect-localwords header-end))
            (save-excursion
              (save-match-data
                (let ((case-fold-search t)
                      last-eol)

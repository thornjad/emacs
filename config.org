...
When multiple frames are open and tab-bar-mode is active, the focused frame gets a Spacemacs-purple tab bar background to make it immediately obvious which frame has focus. Inactive frames keep the default muted tab bar. The effect only activates with two or more visible frames.

#+BEGIN_SRC emacs-lisp :lexical t
  (defvar aero/--tab-bar-active-bg "#655370"
    "Spacemacs purple background for the active frame's tab bar.")
  (defvar aero/--tab-bar-active-fg "#fefbf2"
    "Light foreground for readability against the active tab bar.")

  (defun aero/--highlight-tab-bar (frame)
    "Apply accent tab bar to FRAME."
    (set-face-attribute 'tab-bar frame
                        :inherit nil
                        :background aero/--tab-bar-active-bg
                        :foreground "#d5cdd9")
    (set-face-attribute 'tab-bar-tab frame
                        :inherit nil
                        :background "#7c6b86"
                        :foreground aero/--tab-bar-active-fg
                        :overline aero/--tab-bar-active-fg
                        :weight 'bold)
  ...
  (defun aero/--update-tab-bar-highlight ()
    ...
    (when (bound-and-true-p tab-bar-mode)
      (let* ((frames (seq-filter #'frame-visible-p (frame-list)))
        ...
        (if (<= (length frames) 1)
            (dolist (f frames) (aero/--unhighlight-tab-bar f))
          (dolist (f frames)
            (if (eq f active)
...
#+begin_src emacs-lisp :lexical t
  (package! aero-claude :localpackage
    :commands (aero/claude)
    :init (aero-leader-def "ac" 'aero/claude))
#+end_src

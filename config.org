...
** Side-window management with auto side windows
This is temporarily disabled while I figure out some other solution that works better with my flow. The side windows this creates are difficult to navigate between, configuration is cumbersome, and I don't use it for many things anyway.

Emacs has great functionality built in for side-windows, but its inconsistent. Before using this package I had custom settings sprinkled all over this config, that mostly worked but yet not always. Thankfully this handles it all for me.

#+BEGIN_SRC emacs-lisp :lexical t
  (package! auto-side-windows "MArpogaus/auto-side-windows"
    :disabled
    :after (general)
    :hook (after-init . auto-side-windows-mode)

    :custom
    (auto-side-windows-common-window-parameters '((no-other-window . t) (tab-line-format . none)))
    ...
    (auto-side-windows-bottom-buffer-modes
     '(flymake-diagnostics-buffer-mode eshell-mode shell-mode term-mode vterm-mode comint-mode debugger-mode))
    (auto-side-windows-top-alist '((window-height . 12)))

    (auto-side-windows-right-buffer-names
     '("^\\*eldoc.*\\*$" "^\\*info\\*$" "^\\*Metahelp\\*$"))
    (auto-side-windows-right-buffer-modes
     '(Info-mode eldoc-mode help-mode helpful-mode shortdoc-mode))
    ...
    (window-sides-slots '(1 1 1 1)) ; allow one window per side
    )
#+END_SRC

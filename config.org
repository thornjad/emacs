...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org :builtin
    ...
    ;; disable the element cache entirely. agenda files get rewritten
    ;; out-of-band (Syncthing, org-roam, automated routines), which desyncs
    ;; the cache from the buffer; on this Emacs build that desync makes
    ;; org-element storm warnings instead of reparsing, pegging a core in
    ;; unbreakable C-level GC until memory runs out (seen twice: Jun 2026,
    ;; Jul 2026). Nothing here depends on the cache's speed -- org-roam uses
    ;; its own SQLite DB, and reparsing these files is sub-second anyway.

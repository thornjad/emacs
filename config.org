...
#+BEGIN_SRC emacs-lisp :lexical t
  (defun aero/org-roam-insert-created-property ()
    ...
    (interactive)
    (when (and (org-roam-file-p) (string-suffix-p ".org" (buffer-name buffer)))
...
Also set up Deepseek connection if the API key is available. This is banned in my job because China is scary, but it's actually better than GPT for many things, so it's what I tend to use outside of work.
...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! gptel "karthink/gptel"
    ...
    (when (and (boundp 'deepseek-api-key) deepseek-api-key)
      (gptel-make-deepseek "DeepSeek" :key deepseek-api-key :stream t))
...
** Aider
Unfortunately Cursor is not available for integration, at least as of May 2025, so we're trying out this alternative called Aider.

Most interaction is done through =SPC a i a=, after starting the Aider process. The only major configuration needed is an overridden commit message so it doesn't use "conventional commit" format.

#+BEGIN_SRC emacs-lisp :lexical t
  (package! aider "tninja/aider.el"
    :after (general)
    :functions (aider-transient-menu
                aider-run-aider
    ...
    :custom
    (aider-args '("--model" "sonnet" "--no-auto-accept-architect" "--no-gitignore"))
    :init
    (aero-leader-def
      "aia" 'aider-transient-menu
      "aib" 'aider-switch-to-buffer
      "aif" 'aider-function-or-region-refactor
      "ait" 'aider-implement-todo
      "aiu" 'aider-write-unit-test
      "aiq" 'aider-ask-question
      "aig" 'aider-go-ahead)
    :config
    (setenv "AIDER_COMMIT_PROMPT" "You are an expert software engineer that generates concise, one-line Git commit messages based on the provided diffs.
  Review the provided context and diffs which are about to be committed to a git repo.
  Review the diffs carefully.
  Generate a one-line commit message for those changes.
  You MUST NEVER prefix a commit message with a commit type. Conventional commit format is forbidden.

  Ensure the commit message:
  - Does not begin with a type
...
** Claude Code
Interface for working with Claude from Emacs, with context about what I'm looking at.

Haven't actually gotten this working yet.

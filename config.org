...
#+BEGIN_SRC emacs-lisp :lexical t
  (package! org-roam :auto
    ...
    :config
    ;; Enable automatic database syncing with incremental updates
    ;; This uses file system watchers and updates only modified files,
    ;; avoiding the blocking behavior of full manual syncs with 500+ nodes.
